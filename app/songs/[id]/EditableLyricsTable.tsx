...
import { updateLyricText, updateLyricTiming } from "./actions";

function msToLabel(ms: number): string {
  return (ms / 1000).toFixed(1);
}

export function EditableLyricsTable({
  lyrics,
  hasTimestamps,
}: {
  lyrics: Lyric[];
  ...
  return (
    <table className="w-full text-sm">
      ...
      <tbody>
        {lyrics.map((line) => (
          <LyricRow key={line.id} line={line} hasTimestamps={hasTimestamps} />
        ))}
      </tbody>
    </table>
  );
    ...
    startTransition(async () => {
      try {
        if (hasTimestamps) {
          ...
          await updateLyricText(line.id, text);
        }
        setSaved(true);
      } catch (err) {
