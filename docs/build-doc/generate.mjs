...
const BLOCKS = [
  ...
  para("Every user-owned table (songs, lyrics, clip_segments, payments, exports, profiles) has Row-Level Security enabled with an auth.uid() = user_id policy. Adding a new user-scoped table requires an accompanying RLS migration in the same change. The service-role key bypasses RLS and is used only server-to-server (the Stripe webhook and the free-song claim), never in a request carrying a user session."),
  h2("The download access model"),
  para("Centralised in lib/access.ts and enforced in the export-download route. A clip is downloadable free when the account is a founder (is_founder = true), or it is the user's one claimed free song (profiles.free_song_id, set once by trusted server code), or a paid payment exists for the song. The is_founder and free_song_id columns are locked at the database grant level so a user cannot self-promote or rotate their free song."),
  h2("The security suite"),
  para("tests/security/ runs on every push/PR via GitHub Actions. It covers data isolation, SQL-injection prevention, brute-force rate-limiting, data-exfiltration prevention, and privilege-escalation prevention (17 tests). It runs against the real Supabase project with disposable users that are cleaned up after each run."),
